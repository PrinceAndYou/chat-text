...
//header
$.ajax({
    url:"static/chatSystem/json/chat.json",
...
$.ajax({
    type:"GET",
    url:"static/chatSystem/data/addPerson.json",
    dataType:"json",
    success:function(data){
        var html = '';
        // console.log(data);
        $.each(data,function(key,val){
            ...
            html +=
                ...
                '<dl>'+
                '<dt>'+
                '<img src="static/chatSystem/img/p1_l.jpg" alt="">'+
                '</dt>'+
                '<dd>'+val.name+'</dd>'+
                '</dl>'+
                '</li>';
...
var boxRight = {
    ...
    preloadEvent:function(){
        ...
        $('.biaoqing').qqFace({
            ...
            path:'static/chatSystem/img/arclist/'    //表情图片存放的路径
        });
    },
    // 功能2 单击群名变成修改状态
    ...
    sendTalkEvent:function(){
        ...
        function replace_em(str){
            ...
            str = str.replace(/\[em_([0-9]*)\]/g, '<img src="static/chatSystem/img/arclist/$1.gif" border="0" />');
        ...
        if ($.trim(talkHtml) === '') {
            ...
            html =
                '<ul class="my_talk">'+
                '<li>'+
                '<img src="static/chatSystem/img/p2_l.jpg" alt="">'+
                '</li>'+
                '<li>'+
    ...
    searchFriendEvent:function(){
        ...
        $('#addFriend button').click(function(){
            $.ajax({
            	type: 'POST',
            	url: basePath + 'chatSystemController/findFriendsByInput',
            	data: {
                	"INPUT":input.val()
                },
            	dataType:"json",
                    ...
                    if(data != null){
                    	$.each(data,function(i){
                    			html +=
                    				'<li>'+
                    				'<div>'+
                    				'<img src="'+data[i].IMG_MANE+'"/>'+
                    				'<p>'+
                    				'<span>'+data[i].YHXM+'</span>'+
                    				'<br/>'+
                    				'<span>'+data[i].YHM+'</span>'+
                    				'</p>'+
                    				'</div>'+
                    				'<button class="addBtn" onclick="addFriendBtn();" id="addFriendBtn">添加</button>'+
                    				'</li>';
                    	});
                    }else{
                  		html = '<span style="color:red;font-size:.16rem">未找到查询朋友，请检查后输入!</span>'
                    }
    ...
    searchGroupEvent:function(){
        ...
        $('#addQun button').click(function(){
            $.ajax({
                type:"GET",
                url:"static/chatSystem/data/searchPerson.json",
                dataType:"json",
                success:function(data){
                    html = '';
                    //置空
                    ...
                    $.each(data,function(key,val){
                        ...
                        if(val.name == input.val() || val.num == input.val()){
                            a = b+1;
                            // 得到正确信息的下标 从0开始
                            var index = a - 1;
                            // console.log(a);
                            ...
                            html =
                                ...
                                '</p>'+
                                '</div>'+
                                '<button class="addBtn" id="addGroupBtn" onclick="addGroupBtn();">添加</button>'+
                                '</li>';
                        }else{
                            b++;
                        }
                    ...
                    if(a!==0){
                        ...
                        $('#addQun .searchContent').html('<span style="color:red;font-size:.16rem">未找到相匹配的群，请检查后输入!</span>');
                    }
                }
            });
        });
    ...
    fileViewEvent:function(){
        ...
        $.ajax({
            type:"GET",
            url:"static/chatSystem/data/searchFile.json",
            dataType:"json",
            success:function(data){
                html = '';
                $.each(data,function(key,val){
                    html +=
                        ...
                        '<img src="static/chatSystem/img/stop.gif" alt="">'+
                        '<img src="static/chatSystem/img/delete.gif" alt="">'+
                        '</li>'+
                        '</ul>'+
                        '</td>'+
                        '<td>'+val.user+'</td>'+
        ...
        $('.file-table tbody').on('click','.downloadLogo',function(){
            ...
            var timer = setTimeout(function(){
                ...
                // logo替换
                // console.log($(this));
                $(this).attr('src','static/chatSystem/img/file.gif').removeClass('downloadLogo').addClass('fileLogo');
            }.bind(this),2000);
...
var addFriendBtn =  function () {
    ...
    layer.open({
        ...
        area:['4.5rem','2.5rem'],
        btn:"确认",
        content:'<div style="text-align: center;margin-top: .8rem;"><span><i class="layui-icon">&#x1005;</i>好友请求已发送成功，正在等待对方确认！</span></div>'
